...
GLOBAL_QA_FILE="${ROOT_DIR}/del_global_qa.jsonl"  # 全局QA文件（JSONL）
GLOBAL_REF_FILE="${ROOT_DIR}/del_global_references.jsonl"  # 全局参考文件（JSONL）
INVALID_QA_FILE="${ROOT_DIR}/del_invalid_qa.jsonl"  # 无效数据文件（JSONL，存储answer在question中的原始数据）
PYTHON_SCRIPT="final_del.py"  # Python脚本路径（不在当前目录则写绝对路径）
